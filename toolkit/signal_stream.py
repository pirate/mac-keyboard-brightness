...
MAGIC = b"MSIG1 "
_RATE_PREFIX_RE = re.compile(rb"^\s*([0-9]+(?:\.[0-9]+)?)")
...
class FloatSignalReader:
    ...
    def iter_chunks(self, chunk_bytes: int = 16384) -> Iterator[np.ndarray]:
        ...
        reader = read1 if callable(read1) else self.stream.read
        while True:
            data = reader(chunk_bytes)
            if not data:
                break
            if self.carry:
    ...
    if prefix == MAGIC:
        ...
        for _ in range(64):
            ...
            if not ch:
                break
            if ch == b"\n":
                token = bytes(rate_bytes).strip()
                if not token:
                    raise StreamFormatError("missing sample rate in header")
                try:
                    rate = float(token.decode("ascii", errors="strict"))
        ...
        # Fallback for malformed headers without a newline: salvage a numeric prefix
        # and keep over-read bytes as sample payload.
        m = _RATE_PREFIX_RE.match(bytes(rate_bytes))
        if m:
            try:
                rate = float(m.group(1).decode("ascii", errors="strict"))
            except ValueError as exc:
                raise StreamFormatError("invalid sample rate in header") from exc
            if rate <= 0:
                raise StreamFormatError("sample rate must be > 0")
            return rate, bytes(rate_bytes[m.end() :])
