...
class LevelFollower:
    """Smoothed absolute-level follower for direct amplitude mapping."""

    def __init__(self, *, attack_ms: float, release_ms: float, gain: float) -> None:
        self.attack_s = max(1e-4, float(attack_ms) / 1000.0)
        self.release_s = max(1e-4, float(release_ms) / 1000.0)
        self.gain = max(0.0, float(gain))
        self.env = 0.0
        self.noise = 0.0
        self.excess_peak = 0.02
        self.out = 0.0
        # Let noise floor rise very slowly so sustained loud sections keep driving.
        self.noise_rise_s = 24.0
        self.noise_fall_s = 1.2
        self.excess_peak_decay_s = 2.6
        self.min_excess = 0.0012
    ...
    def update(self, sample: float, dt: float) -> float:
        x = abs(float(sample))
        tau = self.attack_s if x > self.env else self.release_s
        self.env = self._lpf(self.env, x, dt, tau)
        noise_tau = self.noise_rise_s if self.env > self.noise else self.noise_fall_s
        self.noise = self._lpf(self.noise, self.env, dt, noise_tau)

        # Persistent loudness above ambient floor (not just rising transients).
        gate = max(0.00025, self.noise * 1.02)
        excess = max(0.0, self.env - gate)
        peak_decay = math.exp(-max(0.0, dt) / max(1e-6, self.excess_peak_decay_s))
        self.excess_peak = max(excess, self.excess_peak * peak_decay)
        ref = max(self.min_excess, self.excess_peak)
        norm = clamp01(excess / ref)
        target = clamp01((norm ** 0.65) * self.gain * 1.6)
        out_tau = 0.05 if target > self.out else 0.30
        self.out = self._lpf(self.out, target, dt, out_tau)
        if self.out < 0.0015:
            return 0.0
        return self.out
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "direct uses smoothed absolute level; beat uses onset-following envelope (default: auto)."
        ),
    )
    parser.add_argument(
        "--beat-threshold",
...
def main() -> int:
    ...
    if not controller.available:
        ...
        if os.geteuid() != 0 and (
            diag.startswith("IOServiceOpen kr=") or diag.startswith("AppleSMC kr=")
        ):
            require_root(__file__)
        raise SystemExit(
            f"fan-speed control unavailable ({diag}; run as root on Apple Silicon)"
        )
    if controller.backend == "applesmc-iokit" and os.geteuid() != 0:
        require_root(__file__)
    ...
    try:
        ...
        for chunk in reader.iter_chunks(chunk_bytes=chunk_bytes):
            ...
            for sample in chunk:
                ...
                if map_mode == "direct":
                    env = level_follower.update(raw, dt)
                    hold_decay = math.exp(-max(0.0, dt) / max(1e-6, direct_hold_tau))
                    direct_hold_env = max(env, direct_hold_env * hold_decay)
                    env = direct_hold_env
                    last_raw_env = level_follower.env
                    last_noise = level_follower.noise
                    last_peak = level_follower.excess_peak
                else:
                ...
                if span >= 1.0:
                    ...
                    print(
                        (
                            ...
                            f"raw={last_raw_env:.3f} noise={last_noise:.3f} peak={last_peak:.3f} "
        ...
        msg = str(exc)
        if os.geteuid() != 0 and "AppleSMC kr=" in msg:
            require_root(__file__)
