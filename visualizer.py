...
class LiveStats:
    ...
    def _detect_heartbeat(self) -> None:
        ...
        # Keep only finite candidates to avoid NaN propagating into selection.
        cand = [c for c in cand if math.isfinite(c[0]) and math.isfinite(c[1])]
        if not cand:
            self.hr_freq_raw = None
            self.hr_bpm = None
            self.hr_freq_hz = None
        ...
        max_r = max(c[1] for c in cand)
        strong = [c for c in cand if c[1] >= max_r * 0.82]
        if not strong:
            strong = [max(cand, key=lambda t: t[1])]
...
def main() -> int:
    ...
    # Avoid looking "frozen" when upstream never emits an MSIG1 header.
    if not os.isatty(sys.stdin.fileno()):
        print(
            "visualizer: waiting for upstream stream header (MSIG1)...",
            file=sys.stderr,
            flush=True,
        )
