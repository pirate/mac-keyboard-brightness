...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            ...
            "as JSON lines. When stdout is piped, the input stream is passed through "
            "on stdout and JSON is emitted to stderr."
        )
    )
    p.add_argument(
        "--interval",
...
def emit_line(
    *,
    ...
    print(json.dumps(payload, separators=(",", ":")), file=out_stream, flush=True)
...
def main() -> int:
    ...
    stdout_piped = not os.isatty(sys.stdout.fileno())
    json_stream = sys.stderr if stdout_piped else sys.stdout
    writer = FloatSignalWriter.to_stdout(sample_rate=reader.sample_rate, raw=args.raw) if stdout_piped else None
